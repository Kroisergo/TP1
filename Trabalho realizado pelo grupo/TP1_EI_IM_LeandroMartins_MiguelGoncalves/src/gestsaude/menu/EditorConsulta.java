...
public class EditorConsulta extends JDialog {
	...
	private EditorConsulta(Point posicao, GEstSaude g, Consulta consulta, String titulo) {
		...
		// FEITO usar uma lista com os ids das especialidades
		ArrayList<String> especialidadeIds = new ArrayList<>();
		for (Especialidade e : gest.getEspecialidades()) {
			especialidadeIds.add(e.getIdentificador());
		}
		setupAspeto(especialidadeIds);

		// se for uma consulta existente é preciso carregar os dados desta
		if (consulta != null) {
			// FEITO colocar os dados certos nas variáveis
			String snsUtente = "" + consulta.getUtente().getNrSns();
			String idEspecialidade = consulta.getEspecialidade().getIdentificador();
			data = consulta.getDataMarcacao().toLocalDate();
			hora = consulta.getDataMarcacao().toLocalTime();
	...
	protected void testaIdUtente() {
		// FEITO ver se o utente existe
		utente = null;
		try {
			int sns = Integer.parseInt(snsUtenteTF.getText());
			utente = gest.getUtente(sns);
		...
		if (utente == null) {
			...
			// FEITO substituir texto pelo nome do utente
	...
	protected void testaIdServico() {
		// FEITO ver se o id da especialidade escolhida é válido
		...
		if (esp == null) {
			...
			especialidade = esp;
			// FEITO substituir texto pela descrição da especialidade
	...
	private void testaTudoOk() {
		if (utente == null)
			...
			// FEITO criar a consulta e verificar se pode ser adicionada/alterada
			Consulta c = new Consulta(LocalDateTime.of(data, getHora()), utente, especialidade);
	...
	protected void okPremido() {
		...
		// FEITO criar a consulta com os valores escolhidos
