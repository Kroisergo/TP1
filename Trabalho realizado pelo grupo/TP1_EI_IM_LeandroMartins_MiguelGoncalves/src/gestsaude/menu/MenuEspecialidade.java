...
public class MenuEspecialidade extends JDialog {
	...
	private boolean proximoUtente() {
		...
		// FEITO colocar a info nas variáveis
	...
	private void encaminhar() {
		...
		do {
			...
			int resultado = JOptionPane.showConfirmDialog(this, scroll, "Encaminhar para onde?",
					JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
			if (resultado != JOptionPane.OK_OPTION) {
				break;
			}
			String res = lista.getSelectedValue();
			if (res == null || res.isEmpty())
				break;
			// FEITO ver se o serviço existe
			Servico s = gest.getServico(res);
			if (s == null)
				JOptionPane.showMessageDialog(this, "Esse serviço não existe!");
			else {
				// FEITO associar o serviço à senha e a senha aos serviços
				try {
					senha.addServico(s);
	...
	private void listarSenhas() {
		...
		for (Senha s : senhas) {
			// FEITO colocar a info certa nas variáveis
			String numeroSenha = s.getNumero();
			String nomeUtente = s.getConsulta().getUtente().getNome();
			LocalTime hora = s.getHoraPrevistaAtendimento().toLocalTime();
	...
	public void atualizarInfo() {
		// FEITO colocar a info certa
		String idEspecialidade = especial.getIdentificador();
